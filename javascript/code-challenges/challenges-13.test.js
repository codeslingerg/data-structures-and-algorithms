...
if (!Array.isArray(arr) || arr.length === 0) {
  return -1; // Return -1 for invalid input or empty array
}
...
const firstLetters = (arr) => {
  // Solution code here...
  if (!Array.isArray(arr) || arr.length === 0) {
    return []; // Return an empty array for invalid input or empty array
  }

  const firstLetterArray = arr.map((str) => str.charAt(0));
...
const findHappiness = (arr) => {
  ...
  const happyStrings = arr.filter((str) => str.includes(":)"));
  return happyStrings;
};
...
const findAnything = (arr, target) => {
  ...
  const foundStrings = arr.filter((str) => str.includes(target));
  return foundStrings;
};
